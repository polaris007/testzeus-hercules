...
class BaseNavAgent:
    ...
    def __init__(self, model_config_list, llm_config_params: dict[str, Any], system_prompt: str | None, nav_executor: autogen.UserProxyAgent, agent_name: str = None, agent_prompt: str | None = None):  # type: ignore
        ...
        # DEBUG: Log the final llm_config before creating agent
        logger.info("=" * 80)
        logger.info(f"DEBUG: Creating ConversableAgent '{agent_name}'")
        logger.info(f"llm_config keys: {list(llm_config.keys())}")
        
        config_list = llm_config.get('config_list', [])
        if config_list and len(config_list) > 0:
            first_config = config_list[0]
            logger.info(f"First config in config_list:")
            ...
            logger.warning("config_list is EMPTY!")

        logger.info(f"Top-level api_key: {'SET' if llm_config.get('api_key') else 'NOT SET'}")
        logger.info("=" * 80)
