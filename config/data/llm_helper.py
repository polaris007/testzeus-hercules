...
def convert_model_config_to_autogen_format(
    ...
    # DEBUG: Log the exact config being passed to autogen
    logger.info("=" * 80)
    logger.info("DEBUG: Model Config Conversion")
    logger.info(f"Original model_config keys: {list(model_config.keys())}")
    logger.info(f"Converted config_for_autogen:")
    for key, value in config_for_autogen.items():
        if key == 'api_key':
            logger.info(f"  {key}: {value[:10]}...{value[-5:] if len(value) > 15 else ''}")
        else:
    ...
    if active_proxies:
        logger.warning(f"⚠️  Active proxy variables detected: {active_proxies}")
    else:
    ...
    # Also log what autogen returns
    result = autogen.config_list_from_json(env_or_file=temp_file_path)
    logger.info(f"Autogen config_list result (first item):")
    ...
    # Test connectivity to the configured base_url
    base_url = config_for_autogen.get('base_url', '')
    if base_url:
        logger.info(f"\nTesting connectivity to LLM API: {base_url}")
        try:
            ...
            # Test the models endpoint (lightweight)
            ...
            req = urllib.request.Request(
                test_url,
                headers={
                    'Authorization': f"Bearer {config_for_autogen.get('api_key', '')}",
                    'User-Agent': 'Hercules-Debug/1.0'
                },
                method='GET'
            )
            
            response = urllib.request.urlopen(req, timeout=10, context=ctx)
            logger.info(f"  ✓ Connectivity test PASSED: HTTP {response.status}")
            logger.info(f"  Response preview: {response.read(200)}")
        except Exception as e:
            logger.error(f"  ✗ Connectivity test FAILED!")
            logger.error(f"  Error type: {type(e).__name__}")
            logger.error(f"  Error message: {e}")
            logger.error(f"  ")
            logger.error(f"  This means Python cannot reach {base_url}")
            logger.error(f"  Possible causes:")
            logger.error(f"    1. DNS resolution failure")
            logger.error(f"    2. Firewall blocking the connection")
            logger.error(f"    3. Network routing issue")
            logger.error(f"    4. SSL/TLS certificate problem")
            logger.error(f"  ")
            logger.error(f"  Try running in container:")
            logger.error(f"    curl -v {base_url}/models")
            logger.error(f"    nslookup {base_url.split('//')[1].split('/')[0]}")
    logger.info("=" * 80 + "\n")
