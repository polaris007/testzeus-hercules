...
def convert_model_config_to_autogen_format(
    ...
    # DEBUG: Print the config being passed to autogen
    print(f"\n\n=== DEBUG: Model Config for Autogen ===", file=sys.stderr)
    print(f"Original model_config: {model_config}", file=sys.stderr)
    print(f"Converted config_for_autogen: {config_for_autogen}", file=sys.stderr)
    print(f"Final env_var: {env_var}", file=sys.stderr)
    print(f"=====================================\n\n", file=sys.stderr)
    ...
    logger.info(f"Original model_config: {json.dumps(model_config, indent=2, default=str)}")
    logger.info(f"Converted config_for_autogen: {json.dumps(config_for_autogen, indent=2, default=str)}")
    logger.info(f"Has 'base_url': {'base_url' in config_for_autogen}")
    logger.info(f"Base URL value: {config_for_autogen.get('base_url', 'NOT SET')}")
    logger.info("=" * 80)
